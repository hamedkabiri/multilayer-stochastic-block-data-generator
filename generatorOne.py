...
def Multi_deg_generator():
    ...
    np.savetxt('DataCommualpha=1.csv', myData, fmt='%d', delimiter=',')
            ...
            if m == l:
                ...
                for i in range(0, QM[m]):
                    if tabdil[i] in GM[m] & GM[l]:
                        ...
                        ssx[i] = (mu[m] * rokolmm * len(members[m, tabdil[i]]) * (T[m] - len(members[l, tabdil[i]]))) - (rokolmm * len(members[m, tabdil[i]]) * len(members[l, tabdil[i]]))
                ...
                for i in range(0, QM[m]):
                    ...
                    s[m, l, tabdil[i], tabdil[i]] = rokolmm*len(members[m,tabdil[i]])*len(members[l,tabdil[i]]) + fty
                    ...
                    for j in range(0, QM[m]):
                        if i != j:
                                ...
                                s[m, l, tabdil[i], tabdil[j]] = rokolmm*len(members[m,tabdil[i]])*len(members[l,tabdil[j]]) - x_lstsq[ui[i, j]]
                                if s[m, l, tabdil[i], tabdil[j]] > (len(members[m, tabdil[i]]) * len(members[l, tabdil[j]])):
                ...
                for i in GM[m] & GM[l]:
                    ...
                    ssx[tabdil[i]] = (pi[m, l] * densml * len(members[m, i]) * (T[l] - len(members[l, i])))+(pi[l, m] * densml * len(members[l, i]) * (T[m] - len(members[m, i]))) - 2*(densml * len(members[m, i]) * len(members[l, i]))
                    for j in GM[m]|GM[l]:
                        if i !=j :
                            if j in GM[m] & GM[l]:
